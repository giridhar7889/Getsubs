...
osd_username = "giridhar7889"
osd_password = "Giridhar@2339"
osd_language = "en"
...
def superPrint(priority, title, message):
    ...
    if opt_gui == "gnome":
        ...
        subprocess.call(
            [
                "kdialog",
                "--geometry=" + str(opt_gui_width) + "x" + str(opt_gui_height),
                "--title=" + title,
                "--" + priority + "=" + message,
            ]
        )
...
def selectionGnome(subtitlesList):
    ...
    if subtitlesMatchedByName == 0:
        ...
        textstr = (
            ' --text="Search results using file name, NOT video detection. <b>May be unreliable...</b>\n<b>File name:</b> '
            + videoFileName
            + '" '
        )
        ...
        tilestr = ' --title="Subtitles for: ' + videoTitle + '"'
        textstr = (
            ' --text="Search results using file name AND video detection.\n<b>Video title:</b> '
            + videoTitle
            + "\n<b>File name:</b> "
            + videoFileName
            + '"'
    ...
    process_subtitlesSelection = subprocess.Popen(
        "zenity --width="
        + str(opt_gui_width)
        + " --height="
        + str(opt_gui_height)
...
def selectionKde(subtitlesList):
    ...
    if subtitlesMatchedByName == 0:
        ...
        tilestr = ' --title="Subtitles for ' + videoFileName + '"'
        menustr = (
            ' --menu="Search results using file name, NOT video detection. <b>May be unreliable...</b><br><b>File name:</b> '
            + videoFileName
            + '" '
        )
    else:  # a mix of the two
        tilestr = ' --title="Subtitles for ' + videoTitle + '" '
        menustr = (
            ' --menu="Search results using file name AND video detection.<br><b>Video title:</b> '
            + videoTitle
            + "<br><b>File name:</b> "
            + videoFileName
            + '" '
    ...
    while sub_selection < 0 or sub_selection > subtitlesIndex:
        try:
            if sys.version_info >= (3, 0):
                ...
                sub_selection = int(
                    raw_input(">> Enter your choice (0-" + str(subtitlesIndex) + "): ")
                )
        except:
            sub_selection = -1
            ...
            for tool in ["gunzip", "wget"]:
                ...
                if path is None:
                    superPrint(
                        ...
                        "The <b>'"
                        + tool
                        + "'</b> tool is not available, please install it!",
                    )
                    return False
...
parser.add_argument(
    "-g",
    "--gui",
    help="Select the GUI you want from: auto, kde, gnome, cli (default: auto)",
)
parser.add_argument(
    "-l",
    "--lang",
    help="Specify the language in which the subtitles should be downloaded (default: eng).\nSyntax:\n-l eng,fre: search in both language\n-l eng -l fre: download both language",
    nargs="?",
    action="append",
)
parser.add_argument(
    "-i",
    "--skip",
    help="Skip search if an existing subtitles file is detected",
    action="store_true",
)
parser.add_argument(
    "-s",
    "--search",
    help="Search mode: hash, filename, hash_then_filename, hash_and_filename (default: hash_then_filename)",
)
parser.add_argument("-t", "--select", help="Selection mode: manual, default, auto")
parser.add_argument(
    "-a",
    "--auto",
    help="Trigger automatic selection and download of the best subtitles found",
    action="store_true",
)
parser.add_argument(
    "-o",
...
parser.add_argument(
    "filePathListArg",
    help="The video file(s) for which subtitles should be searched and downloaded",
    nargs="+",
)
...
if opt_gui == "auto":
    ...
    ps = str(
        subprocess.Popen(["ps", "cax"], stdout=subprocess.PIPE).communicate()[0]
    ).split("\n")
    for line in ps:
        if (
            ("gnome-session" in line)
            or ("cinnamon-sessio" in line)
            or ("mate-session" in line)
            or ("xfce4-session" in line)
        ):
            opt_gui = "gnome"
            break
...
try:
    ...
    try:
        session = osd_server.LogIn(
            osd_username, osd_password, osd_language, "opensubtitles-download 4.1"
        )
    except Exception:
    ...
    if session["status"] != "200 OK":
        superPrint(
            "error",
            "Connection error!",
            "Opensubtitles.org servers refused the connection: "
            + session["status"]
            + ".\n\nPlease check:\n- Your Internet connection status\n- www.opensubtitles.org availability\n- Your downloads limit (200 subtitles per 24h)\n\nThe subtitles search and download service is powered by opensubtitles.org. Be sure to donate if you appreciate the service provided!",
        )
        sys.exit(2)
    ...
    for SubLanguageID in opt_languages:
        ...
        if (opt_search_mode == "hash_then_filename") and (
            ...
            try:
                ...
                time.sleep(3)
                try:
                    subtitlesList = osd_server.SearchSubtitles(
                        session["token"], searchList
                    )
                except Exception:
                    superPrint(
                        "error",
        ...
        if ("data" in subtitlesList) and (subtitlesList["data"]):
            ...
            if (len(subtitlesList["data"]) == 1) and (
                subtitlesList["data"][0]["MatchedBy"] == "moviehash"
            ):
            ...
            if subtitlesSelected:
                ...
                subLangId = (
                    opt_language_separator + subtitlesList["data"][subIndex]["ISO639"]
                )
                ...
                if opt_gui == "gnome":
                    process_subtitlesDownload = subprocess.call(
                        ...
                        + subtitlesList["data"][subIndex]["LanguageName"]
                        + "</b> subtitles for <b>"
                        + videoTitle
                        + '</b>...")',
                        shell=True,
                    ...
                    process_subtitlesDownload = subprocess.call(
                        "(wget -q -O - " + subURL + " | gunzip > " + subPath + ") 2>&1",
                        shell=True,
                    )
                else:  # CLI
                    print(
                ...
                if process_subtitlesDownload != 0:
                    superPrint(
                        "error",
                        "Subtitling error!",
                        "An error occurred while downloading or writing <b>"
                        + subtitlesList["data"][subIndex]["LanguageName"]
                        + "</b> subtitles for <b>"
                        + videoTitle
                        + "</b>.",
