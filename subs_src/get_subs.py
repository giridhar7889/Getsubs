...
class GetSub:
    ...
    def align(self, video_file_path, srt_path, file_dir, original_name):
        ...
        df.to_csv(os.path.join(file_dir, original_name + "_error.csv"))
        out_path = os.path.join(file_dir, original_name + "_synced.srt")

        command = "srt fixed-timeshift --input {} --output {} --seconds {}"
        command_list = command.format(srt_path, out_path, best_delay_sec).split(" ")

        subprocess.call(command_list)

        print("output aligned subs to:", out_path)

    def find_best_delay_milliseconds(
        self, bin_arr1, bin_arr2, delay_range_start, delay_range_end
    ):
        ...
        with Bar("Finding Best Delay", max=delay_range_len) as bar:
            for i, delay_by_frames in enumerate(
                range(delay_range_start, delay_range_end)
            ):
                ...
                if percent_change > 0.1:
                    ...
                    print(
                        "stopping early at ",
                        str(int(i / delay_range_len * 100.0)) + "%",
                    )
    ...
    def make_list_length_equal(self, lst1, lst2):
        ...
        return list(lst1) + list(np.zeros(max_len - len_lst1).astype(int)), list(
            lst2
        ) + list(np.zeros(max_len - len_lst2).astype(int))
    ...
    def error(self, bin_arr1, bin_arr2):
        # MAE
        return sum(
            np.array(bin_arr1).astype(int) ^ np.array(bin_arr2).astype(int)
        ) / float(len(bin_arr1))
